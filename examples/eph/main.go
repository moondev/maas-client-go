...
func main() {
	...
	// m2, err := deployMachine(client, ctx2, false)
	// if err != nil {
...
func deployMachine(client maasclient.ClientSetInterface, ctx context.Context, ephemeral bool) (maasclient.Machine, error) {
	...
	machine, err = machine.Deployer().
		SetOSSystem("ubuntu").
		SetDistroSeries("custom/ubuntu-2404-efi-kube-v1.32.4").
		SetEphemeralDeploy(ephemeral).
		Deploy(ctx)

		// machine, err = machine.Deployer().
		// SetOSSystem("ubuntu").
		// SetDistroSeries("custom/u-2204-0-k-1316-0").
		// SetEphemeralDeploy(ephemeral).
		// Deploy(ctx)
